  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center">
      ...
      <div className="max-w-2xl w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl p-8 space-y-8">
        ...
        <div className="space-y-6">
          <div className="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
            <label className="block mb-3 text-lg font-medium text-gray-900 dark:text-white">
              Correctness Probability: <span className="text-blue-600">{reversalThreshold}</span>
            </label>
            <Slider
              value={[reversalThreshold]}
              onValueChange={([value]) => setReversalThreshold(value)}
              max={1}
              step={0.1}
              min={0}
              className="w-full"
        ...
        <Button 
          onClick={() => onStart(reversalThreshold, totalTrials)}
